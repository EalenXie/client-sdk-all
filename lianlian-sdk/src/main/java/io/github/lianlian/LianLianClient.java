package io.github.lianlian;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import io.github.lianlian.dto.AccountDTO;
import io.github.lianlian.dto.TransactionsDTO;
import io.github.lianlian.dto.TransactionsEntryListDTO;
import io.github.lianlian.dto.WithdrawRecordDTO;
import io.github.lianlian.vo.*;
import io.github.vo.*;
...
public class LianLianClient {
    ...
    /**
     * 查询用户收款账户余额
     * https://developer.lianlianglobal.com/docs/llp-api/6e3c95aa7b658-
     */
    public List<BalanceVO> accountBalance() {
        ...
        ResponseEntity<LianLianResp<List<BalanceVO>>> response = restOperations.exchange(uri, method, new HttpEntity<>(null, headersWithSign), new ParameterizedTypeReference<LianLianResp<List<BalanceVO>>>() {
        });
        return getLianLianOkRespBody(response);
    }

    /**
     * 用户账户详情查询
     * https://developer.lianlianglobal.com/docs/llp-api/5f2e875a7a8d2-
     */
    public AccountVO account(AccountDTO dto) {
        ...
        ResponseEntity<LianLianResp<AccountVO>> response = restOperations.exchange(uri, method, new HttpEntity<>(dto, headersWithSign), new ParameterizedTypeReference<LianLianResp<AccountVO>>() {
        });
        return getLianLianOkRespBody(response);
    }
    ...
    /**
     * 资金流水查询
     * https://developer.lianlianglobal.com/docs/llp-api/9250a1f8d5c54-
     */
    public TransactionVO transactions(TransactionsDTO dto) {
        String urlNotHost = getUrlNotHost("/myshop/transaction");
        HttpMethod method = HttpMethod.POST;
        UriComponentsBuilder builder = uriComponentsBuilder(urlNotHost);
        URI uri = builder.build().encode().toUri();
        HttpHeaders headersWithSign = getHeadersWithSign(method.name(), urlNotHost, dto, uri.getQuery());
        ResponseEntity<LianLianResp<TransactionVO>> response = restOperations.exchange(uri, method, new HttpEntity<>(dto, headersWithSign), new ParameterizedTypeReference<LianLianResp<TransactionVO>>() {
        });
        return getLianLianOkRespBody(response);
    }
    ...
    /**
     * 入账交易查询
     * https://developer.lianlianglobal.com/docs/llp-api/e361747518aa8-
     */
    public TransactionEntryVO transactionEntry(TransactionsEntryListDTO dto) {
        ...
        ResponseEntity<LianLianResp<TransactionEntryVO>> response = restOperations.exchange(uri, method, new HttpEntity<>(dto, headersWithSign), new ParameterizedTypeReference<LianLianResp<TransactionEntryVO>>() {
        });
        return getLianLianOkRespBody(response);
    }
    ...
    /**
     * 提现交易查询
     * https://developer.lianlianglobal.com/docs/llp-api/3f7ebd879c7c2-
     */
    public WithdrawRecordVO withdrawRecord(WithdrawRecordDTO dto) {
        ...
        HttpHeaders headersWithSign = getHeadersWithSign(method.name(), urlNotHost, dto, uri.getQuery());
        headersWithSign.setContentType(MediaType.APPLICATION_JSON);
        ResponseEntity<LianLianResp<WithdrawRecordVO>> response = restOperations.exchange(uri, method, new HttpEntity<>(dto, headersWithSign), new ParameterizedTypeReference<LianLianResp<WithdrawRecordVO>>() {
        });
        return getLianLianOkRespBody(response);
    }
    ...
    /**
     * https://developer.lianlianglobal.com/docs/llp-api/ZG9jOjI5ODg2NzEw-
     * 签名字符串 拼接公式: HTTP_METHOD&URI&REQUEST_EPOCH&REQUEST_PAYLOAD[&QUERY_STRING]
     *
    ...
    public HttpHeaders getHeadersWithSign(String method, String urlNotHost, @Nullable Object body, @Nullable String queryParams) {
        ...
        @SuppressWarnings("all") final String signHeader = "LLPAY-Signature";
        ...
        headers.set(signHeader, String.format("t=%s,v=%s", requestEpoch, signSha256Rsa));
        return headers;
    }
    ...
    public PrivateKey getPkcs1PrivateKey(String pkcs1PrivateKeyFilePath) {
        ...
        try (InputStreamReader inputStreamReader = new InputStreamReader(classPathResource.getInputStream()); PEMParser pemParser = new PEMParser(inputStreamReader)) {
            //  RSA PKCS1 私钥读取
